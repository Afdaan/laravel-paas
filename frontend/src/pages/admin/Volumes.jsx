    ...
    if (isLoading && data.volumes.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] gap-6">
                ...
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] animate-pulse">Scanning Storage Cluster</p>
            </div>
        )
    }
            ...
            <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-4 bg-white/[0.02] border border-white/10 p-2 rounded-2xl backdrop-blur-md">
                            <div className="flex items-center gap-3 px-6 py-2 border-r border-white/5">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">{stats.total} Active</span>
                            </div>
                            <div className="flex items-center gap-3 px-6 py-2">
                                <div className="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-[0_0_15px_rgba(249,115,22,0.5)]"></div>
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">{stats.unused} Orphaned</span>
                            </div>
                        </div>
                            ...
                            <tbody>
                                {data.volumes.map((v, i) => (
                                    <tr key={i} className="group hover:bg-white/[0.03]">
                                        ...
                                        <td>
                                            <div className="flex items-center gap-5">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-black text-white group-hover:text-blue-400 transition-colors truncate max-w-[300px] uppercase tracking-tight">{v.name}</span>
                                        ...
                                        <td className="text-center">
                                            <span className="text-[11px] font-mono font-black text-slate-400 bg-white/[0.03] px-3 py-1 rounded-lg border border-white/5">
                                                {v.size || 'N/A'}
                                            </span>
                                        </td>
                                        <td className="text-center text-slate-500 text-[11px] font-black">
                                            -
                                        </td>
                                        <td className="text-center">
                                            <span className="px-3 py-1 rounded-lg bg-black/40 border border-white/5 text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">
                                                {v.driver}
                                            </span>
                                        </td>
                ...
                <div className="mt-8 flex flex-col md:flex-row items-center justify-between gap-6 px-10 py-8 card-glass bg-white/[0.01] border-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Showing {data.volumes.length} results of cluster state.</span>
                    </div>
