...
const StatusBadge = ({ status }) => {
  const configs = {
    ...
    stopped: { 
      color: 'bg-slate-500/5 text-slate-500 border-white/5', 
      label: 'Halted', 
      glow: 'bg-slate-500' 
    },
  }
...
const ResourceBar = ({ label, value, max, icon: Icon, suffix = '' }) => {
  ...
  return (
    <div className="flex flex-col gap-2 w-40 group/res">
      <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-widest text-slate-500 group-hover/res:text-slate-400 transition-colors">
          ...
          <div className="flex flex-col xl:flex-row xl:items-end justify-between mb-12 gap-8">
            ...
            <div className="flex items-center gap-4 bg-white/[0.02] border border-white/10 p-2 rounded-2xl backdrop-blur-md">
                 <div className="flex items-center gap-3 px-6 py-2 border-r border-white/5">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">{total} Monitored</span>
                 </div>
                 <div className="flex items-center gap-3 px-6 py-2">
                    <Monitor className="w-4 h-4 text-indigo-500" />
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">{projects.filter(p => p.status === 'running').length} Active Nodes</span>
                 </div>
            </div>
          </div>
                ...
                <table className="premium-table">
                  ...
                  <tbody>
                    {projects.map((project) => {
                      ...
                      return (
                        <tr key={project.id} className="group hover:bg-white/[0.03]">
                          <td>
                            <div className="flex items-center gap-5">
                              <div className="w-12 h-12 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center text-slate-400 group-hover:border-indigo-500/40 group-hover:text-indigo-400 group-hover:bg-indigo-500/5 transition-all duration-500 overflow-hidden relative">
                              ...
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                                  <span className="text-sm font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight truncate max-w-[180px]">{project.name}</span>
                          ...
                          <td>
                            <div className="flex items-center gap-3">
                              <div className="w-7 h-7 rounded-full bg-white/[0.05] border border-white/5 flex items-center justify-center text-slate-500 group-hover:text-white transition-colors">
                                 <User className="w-3.5 h-3.5" />
                              </div>
                              <div className="flex flex-col">
                                <span className="text-[11px] font-black text-slate-300 uppercase tracking-tight">{project.user?.name || 'Cluster Sync'}</span>
                          ...
                          <td>
                            <div className="flex items-center gap-2">
                                <div className="w-7 h-7 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center text-[10px] font-black text-slate-500 uppercase tracking-tighter group-hover:border-rose-500/20 group-hover:text-rose-400 transition-all">
                                    L
                                </div>
            ...
            {totalPages > 1 && (
              <div className="p-10 border-t border-white/5 bg-white/[0.01] flex flex-col md:flex-row justify-between items-center gap-8">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Showing {(page - 1) * 12 + 1} to {Math.min(page * 12, total)} of {total} indexed services.</span>
                </div>
