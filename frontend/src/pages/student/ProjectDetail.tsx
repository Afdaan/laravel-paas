...
function StatusIndicator({ status }: { status: string }) {
  const styles: Record<string, any> = {
    running: { color: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20', label: 'Active' },
    building: { color: 'text-blue-500 bg-blue-500/10 border-blue-500/20', label: 'Building', pulse: true },
    failed: { color: 'text-rose-500 bg-rose-500/10 border-rose-500/20', label: 'Failed' },
    pending: { color: 'text-amber-500 bg-amber-500/10 border-amber-500/20', label: 'Queued' },
    stopped: { color: 'text-slate-500 bg-slate-500/10 border-slate-500/20 dark:text-slate-400', label: 'Offline' },
  }
      ...
      onConfirm: () => {
        toast.promise(
          projectsAPI.redeploy(id),
          {
            ...
            success: 'Deployment started in background',
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          ...
          toast.success(`PHP version updated to ${newVersion}. Rebuilding...`)
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4">
        ...
        <div className="text-muted-foreground animate-pulse font-medium">Loading project configuration...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-20 animate-in fade-in duration-500">
        ...
        <Card className="border-blue-500/20 bg-blue-500/5 p-6 animate-pulse">
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-bold">Setting Up Your Project</h3>
              <p className="text-sm text-muted-foreground">We are currently building your project environment. Live metrics will initialize soon.</p>
            </div>
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
         ...
         <MetricCard 
            title="Database" 
            value="MySQL" 
            subtext={project.database_name || 'No DB'}
            icon={DatabaseIcon}
         />
         <MetricCard 
            title="Queue Worker" 
            value={project.queue_enabled ? 'Enabled' : 'Disabled'} 
            subtext={project.queue_enabled ? 'Background' : 'Direct'}
      ...
      <Tabs defaultValue="project" onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-muted p-1 rounded-lg w-fit overflow-x-auto">
          ...
          <TabsTrigger value="environment">Secrets</TabsTrigger>
                 ...
                 <CardContent>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-muted/50 rounded-xl border gap-4">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <div className="font-bold text-sm">Production URL</div>
                             <div className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">Web Access • SSL Enabled</div>
                          </div>
                       </div>
              ...
              <Card>
                 ...
                 <CardContent className="space-y-4">
                    <div className="p-3 rounded-lg bg-muted border">
                       <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1 block">URI</label>
                       <div className="text-xs font-mono truncate">{project.github_url || project.repository_url}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                       <div className="p-3 rounded-lg bg-muted border">
                          <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1 block">Branch</label>
                       ...
                       <div className="p-3 rounded-lg bg-muted border">
                          <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1 block">Runtime</label>
        ...
        <TabsContent value="console" className="pt-6">
          <Card className="bg-black text-white border-muted/20 overflow-hidden flex flex-col h-[600px]">
            ...
            <div className="flex-1 p-6 overflow-auto font-mono text-xs text-zinc-300 custom-scrollbar bg-zinc-950/50">
              ...
              {consoleOutput ? consoleOutput.split('\n').map((line, i) => (
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-10 gap-4">
                  ...
                  <p className="uppercase tracking-[0.3em] font-bold">Terminal Ready</p>
                </div>
              )}
              {isExecuting && <div className="mt-4 flex items-center gap-2 text-primary animate-pulse"><RefreshCw className="w-3 h-3 animate-spin"/> Running command...</div>}
              <div ref={logsEndRef} />
            </div>
            ...
            <form onSubmit={handleConsoleSubmit} className="p-4 bg-zinc-900 border-t border-white/5 flex gap-3">
              ...
              />
              <Button type="submit" disabled={isExecuting || !consoleCommand.trim()} size="sm">Execute</Button>
            </form>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="environment" className="pt-6">
          <Card className="flex flex-col h-[600px] overflow-hidden">
            <CardHeader className="pb-4 flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-lg">Environment Secrets</CardTitle>
                <CardDescription>Application environment configuration (.env)</CardDescription>
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="flex-1 relative bg-muted/20">
              ...
              {isEnvHidden && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <Badge variant="secondary" className="px-4 py-2 border shadow-lg tracking-widest uppercase">Secret View Locked</Badge>
                </div>
              )}
            </div>
            <div className="p-3 bg-amber-500/5 text-amber-600 text-[10px] font-bold uppercase tracking-widest border-t flex items-center justify-center gap-2">
              <AlertTriangle size={12} /> Saving secrets will trigger an immediate redeployment.
            </div>
          </Card>
        </TabsContent>
                    ...
                    <div className="flex items-center justify-between">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             ...
                             <CardDescription>Connection parameters for your app</CardDescription>
                          </div>
                       </div>
                       <Badge variant="outline" className="text-emerald-500 bg-emerald-500/5 border-emerald-500/20 gap-1.5 uppercase tracking-wider text-[10px]">
                          <ShieldAlert className="w-3 h-3" /> Private Node Access
                       </Badge>
                    </div>
                 </CardHeader>
                 <CardContent>
          ...
          <Card className="bg-black text-zinc-300 border-muted/20 overflow-hidden flex flex-col h-[600px]">
            ...
            <div className="flex-1 p-6 overflow-auto font-mono text-xs leading-relaxed custom-scrollbar bg-black/40">
              {logs ? logs.split('\n').map((line, i) => (
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-10 gap-4">
                  <Activity size={48} />
                  <p className="uppercase tracking-[0.3em] font-bold">Waiting for logs...</p>
                </div>
              )}
              <div ref={logsEndRef} />
            </div>
              ...
              <Card>
                 <CardHeader>
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <CardTitle className="text-lg">PHP Runtime</CardTitle>
                          <CardDescription>Select language version</CardDescription>
                       </div>
                    </div>
                 </CardHeader>
                       ...
                       >
                          ...
                          <SelectContent>
                             {['8.1', '8.2', '8.3', '8.4'].map(v => (
                                <SelectItem key={v} value={v}>PHP {v} {v === '8.4' ? '(Latest)' : ''}</SelectItem>
                             ))}
                          </SelectContent>
                       </Select>
                       <p className="text-[10px] text-muted-foreground italic pl-1 flex items-center gap-1.5 mt-2">
                          <AlertTriangle size={10} className="text-amber-500" /> Changing version triggers a container rebuild.
                       </p>
                    </div>
                 </CardContent>
              </Card>
                 ...
                 <CardHeader>
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          ...
                          <CardDescription>Queue worker configuration</CardDescription>
                       </div>
                    </div>
                 </CardHeader>
                 <CardContent>
                    <div className="flex items-center justify-between p-4 bg-muted/50 rounded-xl border">
                       <div>
                          <p className="font-bold text-sm">worker:php artisan queue:work</p>
                          <p className="text-[10px] text-muted-foreground">Handles asynchronous tasks</p>
                       </div>
                       <Switch 
