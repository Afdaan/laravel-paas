...
const StudentFeedback = () => {
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      default: return 'text-slate-600 dark:text-slate-400 border-slate-300 dark:border-white/10 bg-slate-100 dark:bg-white/5';
    }
  }
  ...
  return (
    <div className="space-y-16 animate-pop-in relative">
      ...
      <div className="relative z-10">
        <div className="mb-16">
          <h1 className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter mb-4 italic">Support <span className="text-transparent bg-clip-text pr-2 pb-1 bg-gradient-to-r from-indigo-400 to-purple-400">Hub</span></h1>
          <p className="text-slate-600 dark:text-slate-400 text-lg font-medium">Send us your feedback or report bugs to our team.</p>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
          {/* Submission Interface */}
          <div className="xl:col-span-8 space-y-10">
            <div className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm p-10 bg-slate-50 dark:bg-slate-100 dark:bg-white/5 border-slate-300 dark:border-white/10 group overflow-hidden relative transition-all duration-500 hover:bg-slate-50 dark:bg-slate-100 dark:bg-white/5">
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-10">
                  <div className="space-y-3">
                    ...
                    <input 
                      type="text" 
                      className={`w-full bg-black/40 border border-slate-300 dark:border-white/10 rounded-2xl px-6 py-5 text-slate-900 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/40 transition-all outline-none ${validationErrors.title ? '!border-rose-500/50 focus:!ring-rose-500/20 focus:!border-rose-500' : ''}`}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="w-full bg-black/40 border border-slate-300 dark:border-white/10 rounded-2xl px-6 py-5 text-slate-900 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500/40 transition-all outline-none flex items-center justify-between group"
                        >
                          <div className="flex items-center gap-3">
                            <activeType.icon className={`w-5 h-5 ${activeType.color}`} />
                            <span>{activeType.label}</span>
                          </div>
                          <ChevronDown className={`w-4 h-4 text-slate-600 dark:text-slate-400 transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                        ...
                        {isDropdownOpen && (
                          <>
                            ...
                            ></div>
                            <div className="absolute top-full left-0 w-full mt-3 bg-[#0d0d12] border border-slate-300 dark:border-white/10 rounded-2xl p-2 z-50 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in fade-in slide-in-from-top-2 duration-200">
                                ...
                                <button
                                  ...
                                  className={`w-full flex items-center gap-4 px-4 py-4 rounded-xl transition-all ${
                                    formData.type === option.value 
                                      ...
                                      : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:bg-slate-100 dark:bg-white/5 hover:text-slate-200'
                                  }`}
                                >
                                  ...
                                  <div className="text-left">
                                    ...
                                    <p className="text-[10px] text-slate-600 dark:text-slate-400 font-medium uppercase tracking-widest mt-0.5">
                    ...
                    <div className="flex items-center">
                        <div className="p-6 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 flex items-center gap-4 w-full">
                            ...
                            <p className="text-[11px] text-slate-600 dark:text-slate-400 font-medium leading-relaxed italic">Your contribution helps us maintain high uptime across the <span className="text-indigo-400 font-black">platform</span>.</p>
                        </div>
                    </div>
                  </div>
                  ...
                  >
                    {isSubmitting ? (
                      <div className="w-5 h-5 border-2 border-slate-300 dark:border-white/20 border-t-white rounded-full animate-spin" />
                    ) : (
          ...
          <div className="xl:col-span-4 space-y-8">
            <div className="flex items-center justify-between ml-2">
                 <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-[0.2em]">Activity Log</h3>
                 </div>
            ...
            <div className="space-y-6 max-h-[850px] overflow-y-auto pr-2 custom-scrollbar">
              {isLoading ? (
                ...
                <div className="p-16 text-center bg-slate-50 dark:bg-slate-100 dark:bg-white/5 border border-dashed border-slate-200 dark:border-white/5 rounded-3xl flex flex-col items-center gap-5">
                ...
                feedback.map(item => (
                  <div key={item.id} className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm p-8 bg-slate-50 dark:bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/5 group hover:bg-slate-50 dark:bg-slate-100 dark:bg-white/5 transition-all duration-300 relative overflow-hidden">
                    {/* Status Glow */}
                    ...
                    <h4 className="text-base font-black text-slate-900 dark:text-white truncate mb-2 group-hover:text-indigo-400 transition-colors uppercase tracking-tight">{item.title}</h4>
                    <p className="text-xs text-slate-600 dark:text-slate-400 font-medium leading-relaxed italic line-clamp-3 mb-6 pr-4">"{item.content}"</p>
