...
const BuildLogsConsole = ({ projectId }: BuildLogsConsoleProps) => {
  ...
  return (
    <Card className="bg-zinc-950 text-white border-none overflow-hidden flex flex-col h-[600px] gap-0 py-0 shadow-2xl">
      <CardHeader className="bg-zinc-900/50 px-4 py-3 border-b border-white/5 flex flex-row items-center justify-between shrink-0 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setLogs('')}
            className="text-[10px] uppercase font-bold text-zinc-600 hover:text-rose-400 transition-colors px-2"
          >
            {t('projectDetail.actions.clear')}
          </button>
        </div>
      </CardHeader>
      <CardContent className="flex-1 p-0 overflow-hidden bg-zinc-950/50">
        <div
          ref={scrollRef}
