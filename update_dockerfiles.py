...
# Dynamic build command argument (fallback for existing projects)
ARG BUILD_COMMAND="if [ -f bun.lock ]; then bun install && bun run build; elif [ -f package-lock.json ]; then npm ci && npm run build; elif [ -f yarn.lock ]; then yarn install && yarn build; elif [ -f pnpm-lock.yaml ]; then pnpm install && pnpm build; else npm install && npm run build; fi"
...
# Execute user-defined build command
